...
function checkBottom(colorIndex) {
  var colorReport = reportColor(5, colorIndex);
  for (var row = 5; row > -1; row--) {
    colorReport = reportColor(row,colorIndex);
...
function verticalWinCheck() {
  for (var col = 0; row < 7; col++) {
    for (var row = 0; row < 3; row++) {
      if (colorMatchCheck(reportColor(row,col), reportColor(row+1,col), reportColor(row+2,col), returnColor(row+3,col))) {
...
$('h3').text(player1+" it is your turn, pick a colomn to drop")
